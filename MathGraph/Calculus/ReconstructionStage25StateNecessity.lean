...
theorem fullReachabilityEq_no_endogenous_residual
    {Ω : Type u} {G : Ω → Ω → Type v} {x y : Ω}
    (hFull : FullReachabilityEq G x y) :
...
theorem fullReachabilityEq_no_probeResidual
    {Ω : Type u} {G : Ω → Ω → Type v} {x y : Ω}
    (hFull : FullReachabilityEq G x y) :
    ¬ ∃ k : Ω,
...
theorem stage25_selective_state_ablation_kills_first_residual :
    ProbeResidual oneEdgeGenerator
      ...
      true false true ∧
    ¬ (FullReachabilityEq oneEdgeGenerator false true ∧
...
theorem reconstruction_stage25_observational_state_necessity_certificate :
  ...
  intro Ω G x y hFull
  exact fullReachabilityEq_no_endogenous_residual hFull
