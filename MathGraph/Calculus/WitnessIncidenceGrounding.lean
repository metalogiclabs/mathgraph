...
/-- Bare witness-carrying incidence data.  An incidence is now a type of
witnesses, not a `Prop`-valued characteristic relation.  No equality,
comparison, apartness, or truth-valued observation is stored in the data. -/
...
def witnessReflect {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) : Incidence κ α :=
...
def WitnessSame {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) (x y : α) : Prop :=
...
theorem witnessSame_refl
    {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) (x : α) :
...
theorem witnessSame_symm
    {κ : Type w} {α : Type u}
    {W : WitnessIncidence (v := v) κ α} {x y : α}
...
theorem witnessSame_trans
    {κ : Type w} {α : Type u}
    {W : WitnessIncidence (v := v) κ α} {x y z : α}
...
theorem witnessDifferent_cotrans_classical
    {κ : Type w} {α : Type u}
    {W : WitnessIncidence (v := v) κ α} {x z : α}
    (hxz : WitnessDifferent W x z) (y : α) :
    WitnessDifferent W x y ∨ WitnessDifferent W y z := by
  classical
  exact incidenceDifferent_cotrans (R := witnessReflect W) hxz y

/-- Add one new witness family as a new test. -/
def witnessExtend {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) (c : α → Type v) :
    WitnessIncidence (v := v) (Sum κ Unit) α
  | Sum.inl k, x => W k x
  | Sum.inr _, x => c x
...
theorem witness_extension_refines
    {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) (c : α → Type v) {x y : α}
...
theorem empty_witness_incidence_collapses_all
    {α : Type u} (x y : α) :
    WitnessSame (fun k : Empty => nomatch k) x y := by
  intro k
  exact nomatch k
...
def witnessDuplicate {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) : WitnessIncidence (v := v) κ α :=
  fun k x => Sum (W k x) (W k x)

theorem duplicate_occupancy_iff
    {κ : Type w} {α : Type u}
    (W : WitnessIncidence (v := v) κ α) (k : κ) (x : α) :
