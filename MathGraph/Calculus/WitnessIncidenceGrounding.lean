...
theorem witnessSeparated_cotrans_of_decidable
  ...
  · cases dec k y with
    | isTrue hy => exact Or.inr ⟨k, Or.inl ⟨hy, hxz.2⟩⟩
    | isFalse hy => exact Or.inl ⟨k, Or.inl ⟨hxz.1, hy⟩⟩
  · cases dec k y with
    | isTrue hy => exact Or.inl ⟨k, Or.inr ⟨hzx.1, hy⟩⟩
    | isFalse hy => exact Or.inr ⟨k, Or.inr ⟨hy, hzx.2⟩⟩
...
theorem witnessDifferent_cotrans_classical
  ...
  classical
  cases Classical.em (WitnessSame W x y) with
  | inl hxy =>
      right
      intro hyz
      exact hxz (witnessSame_trans hxy hyz)
...
theorem empty_witness_incidence_collapses_all
    {α : Type u} (x y : α) :
    WitnessSame (((fun k : Empty => nomatch k) : Empty → α → Type v)) x y := by
  intro k
  exact nomatch k
