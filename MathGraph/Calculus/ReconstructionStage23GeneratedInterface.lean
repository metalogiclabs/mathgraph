...
def GeneratedInterface (Ω : Type u) := List Ω
...
def GeneratedInterface.language
    ...
    ConsequenceLanguage Ω :=
  ProbedReachabilityLanguage G I.probes
...
/-- Empty generated history induces the same universal consequential identity
as the earlier externally supplied zero-probe interface. -/
theorem generatedInterface_empty_identifies
    {Ω : Type u} {G : Ω → Ω → Type v} (x y : Ω) :
    ConsequentialEq (GeneratedInterface.language G ([] : GeneratedInterface Ω)) x y := by
  intro i
  exact Fin.elim0 i

/-- In the one-way Bool bedrock, the generated history can bootstrap itself:
...
theorem stage23_empty_history_has_endogenous_source :
    ProbeResidual oneEdgeGenerator
      (([] : GeneratedInterface Bool).probes) true false true := by
  refine ⟨generatedInterface_empty_identifies false true, ?_⟩
  intro hEq
  have hFalse : ProbeObservation oneEdgeGenerator true false := hEq.mpr ⟨(.nil : FreePath oneEdgeGenerator true true)⟩
  rcases hFalse with ⟨p⟩
  exact (no_path_true_to_false p).elim
...
theorem stage23_generated_history_strictly_refines :
    Refines
      (GeneratedInterface.language oneEdgeGenerator ([true] : GeneratedInterface Bool))
      (GeneratedInterface.language oneEdgeGenerator ([] : GeneratedInterface Bool)) ∧
    ¬ Refines
      (GeneratedInterface.language oneEdgeGenerator ([] : GeneratedInterface Bool))
      (GeneratedInterface.language oneEdgeGenerator ([true] : GeneratedInterface Bool)) := by
  constructor
  · intro x y _h i
    exact Fin.elim0 i
  · intro h
    ...
    have hNew := h hOld
    have hFalse : ProbeObservation oneEdgeGenerator true false := hNew 0 |>.mpr ⟨(.nil : FreePath oneEdgeGenerator true true)⟩
    rcases hFalse with ⟨p⟩
    exact (no_path_true_to_false p).elim
...
theorem reconstruction_stage23_generated_interface_certificate :
    ProbeResidual oneEdgeGenerator
      (([] : GeneratedInterface Bool).probes) true false true ∧
    (Refines
      (GeneratedInterface.language oneEdgeGenerator ([true] : GeneratedInterface Bool))
      (GeneratedInterface.language oneEdgeGenerator ([] : GeneratedInterface Bool)) ∧
     ¬ Refines
