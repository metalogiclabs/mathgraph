...
/-!
# Finite H-Tilt Coordinate Checklist Master

This fixture is the canonical compressed pre-spectral coordinate theorem.
...
For a finite coordinate list `coords`, if every coordinate satisfies a residual
envelope bound and a positive real-part gap bound, and if the scalar side
conditions `0 < δ`, `0 ≤ c`, and `B/(2δ) < c` hold, then every coordinate in the
list has strictly smaller shifted squared modulus than the target coordinate.

No matrices, eigenvalues, spectral theorem, Perron--Frobenius, irreducibility,
or convergence claims are made.
-/

namespace HTiltCoordinateChecklistMaster
...
end HTiltCoordinateChecklistMaster
