...
/-!
# Finite H-Tilt Spectrum Coordinate Envelope

This fixture introduces a safe pre-spectral coordinate-envelope layer.

It treats a "spectrum" only as a finite list of real coordinate pairs `(u,v)`.
It does not prove that the list comes from a matrix spectrum.

Given a target coordinate `(a,b)`, a residual envelope `B`, a positive
real-part gap lower bound `δ`, and an explicit shift bound `c > B/(2δ)`,
Lean verifies that every coordinate in the finite list is dominated after
the shift.
...
namespace HTiltSpectrumCoordinateEnvelope
...
This is still only finite coordinate-list algebra.
-/
...
If `δ > 0`, `c ≥ 0`, and `B/(2δ) < c`, then the scalar shift condition
`B < 2*c*δ` holds, so every coordinate in the finite list is dominated.
-/
...
end HTiltSpectrumCoordinateEnvelope
