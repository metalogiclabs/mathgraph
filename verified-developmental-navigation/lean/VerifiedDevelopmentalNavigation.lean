...
A deterministic core for verified developmental navigation.
...
def run (W : World X C A O) : List A → X → Option X
  | [], x => some x
  | a :: as, x =>
...
@[simp] theorem run_nil (x : X) : run W [] x = some x := rfl

@[simp] theorem run_cons (a : A) (as : List A) (x : X) :
    run W (a :: as) x = (W.step a x).bind (run W as) := by
...
def FutureEq (x y : X) : Prop :=
  ∀ (trace : List A) (c : C), W.OutcomeEq c (run W trace x) (run W trace y)

theorem futureEq_refl (x : X) : W.FutureEq x x := by
  intro trace c
  cases h : run W trace x <;> simp [OutcomeEq, h]

theorem futureEq_symm {x y : X} (hxy : W.FutureEq x y) : W.FutureEq y x := by
  intro trace c
  have h := hxy trace c
  cases hx : run W trace x <;> cases hy : run W trace y <;>
...
theorem futureEq_trans {x y z : X}
  ...
  have h1 := hxy trace c
  have h2 := hyz trace c
  cases hx : run W trace x <;> cases hy : run W trace y <;> cases hz : run W trace z <;>
...
Every relation safe to quotient by with respect to all protected verified
futures refines `FutureEq`. Thus `FutureEq` is the greatest safe relation, and
its quotient is the coarsest representation preserving those future decisions. -/
...
/-- Restrict future equivalence to a protected family of verifier contexts. -/
def FutureEqOn (S : C → Prop) (x y : X) : Prop :=
  ∀ (trace : List A) ⦃c : C⦄, S c →
    W.OutcomeEq c (run W trace x) (run W trace y)
...
theorem separator_forces_split {S : C → Prop} {x y : X}
    ...
    (hsep : ¬ W.OutcomeEq c (run W trace x) (run W trace y)) :
    ¬ W.FutureEqOn S x y := by
  intro hxy
  exact hsep (hxy trace hc)

/-- A one-step recursive law. With at least one verifier context available,
future-equivalent states cannot disagree about whether an action survives; if
both survive, the successor states remain future-equivalent. -/
theorem futureEq_step [Nonempty C] {x y : X} (hxy : W.FutureEq x y) (a : A) :
