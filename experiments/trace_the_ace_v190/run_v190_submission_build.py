...
sys.path.insert(0,str(ROOT/'experiments'/'trace_the_ace_v186'))
import run_v186_mastery_tournament as v186

WS=Path('/workspace'); WORK=WS/'trace-ace-work/v190'; OUT=WS/'trace-ace-results/v190'; PKG=WORK/'package'
# Reuse V185's completed, content-addressed transcript representation cache.
CACHE=WS/'trace-ace-work/v185/feature_cache.pkl'; ASSET=PKG/'v190_assets.joblib'
EPS=1e-6
...
def main():
    ...
    sm,smed,sn=v186.session_proxy(df,oof,op)
    AG=np.column_stack([A,go,np.tanh(go),np.log1p(gs),go*np.log1p(gs),sm,smed,np.log1p(sn)])
    ...
    assets={'version':'V190_MASTERY_GEOMETRY_SESSION_ABILITY','vectorizer':vec,'scalar_mu':mu,'scalar_sd':sd,'base_model':base,'geometry_meta':geometry_meta,'graph_objects':objs,'graph_index':oi,'graph_W':W,'prior_stats':prior_stats,'global_mean':float(gm),'prior_alpha':2.0,'train_rows':len(df)}
...
A=joblib.load(BASE/'v190_assets.joblib'); EPS=1e-6
...
def main():
    ...
    res=logit(ptext)-logit(pp); sm=np.zeros(len(X)); smed=np.zeros(len(X)); sn=np.zeros(len(X))
    for inds in by.values():
        for i in inds:
            vals=[res[j] for j in inds if j!=i]
            if vals: sm[i]=float(np.mean(vals)); smed[i]=float(np.median(vals)); sn[i]=len(vals)
    B=np.column_stack([logit(ptext),logit(pp),np.log1p(cc),logit(ptext)*np.log1p(cc),logit(pp)*np.log1p(cc)]); BG=np.column_stack([B,sig,np.tanh(sig),np.log1p(sup),sig*np.log1p(sup),sm,smed,np.log1p(sn)])
...
'''
    ...
    zip_path=OUT/'trace_ace_v190_mastery_geometry_session_ability_submission.zip'
    with zipfile.ZipFile(zip_path,'w',zipfile.ZIP_DEFLATED) as z:
        z.write(PKG/'main.py','main.py'); z.write(ASSET,'v190_assets.joblib')
    result={'protocol':'V190_FULL_DATA_SESSION_ABILITY_SUBMISSION_BUILD','train_rows':len(df),'feature_cache_rows':len(cache_obj['texts']),'asset_bytes':ASSET.stat().st_size,'zip_bytes':zip_path.stat().st_size,'graph_objectives':len(objs),'graph_edges':int((cnt>0).sum()),'decision':'ASSETS_BUILT__RUN_EXACT_HARNESS'}
    (OUT/'v190_results.json').write_text(json.dumps(result,indent=2)); print(json.dumps(result,indent=2))
if __name__=='__main__': main()
