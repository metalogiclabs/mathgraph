...
WS=Path('/workspace'); FEAT=WS/'trace_the_ace/train_features_TMQTWsB.csv'; LAB=WS/'trace_the_ace/train_labels_44ujmj2.csv'; TR=WS/'trace_the_ace/transcripts_extracted'; OUT=WS/'trace-ace-results/v179'
...
def find_transcript(sid):
    ...
    # only fallback if archive nested paths survived extraction
...
def build_row(sid,obj,cache):
    ...
    # If no lexical overlap, use terminal interaction as fallback rather than whole transcript.
    if not top: top=list(range(max(0,len(z)-6),len(z)))
    keep=set()
    for i in top:
        for j in range(max(0,i-1),min(len(z),i+2)): keep.add(j)
    ordered=sorted(keep)
    parts=[]; vals=[]; rel_student=[]; rel_tutor=[]
    for i in ordered:
    ...
    last_student=rel_student[-1][1] if rel_student else ''
    first_student=rel_student[0][1] if rel_student else ''
    def student_signal(text): return count_phrases(text,POS)-count_phrases(text,UNC)-0.5*count_phrases(text,SELF)
    ...
    vals=[
      ...
      student_signal(last_student),student_signal(last_student)-student_signal(first_student),
      1.0 if top and max(top)>=len(z)-3 else 0.0,
    ]
    text='OBJECTIVE: '+str(obj)+' [KEY_MOMENTS] '+' '.join(parts)
    return text,np.asarray(vals,float)
...
def main():
    ...
    # Objective-conditioned retrieved text; deliberately compact for fast sniff testing.
    vec=TfidfVectorizer(ngram_range=(1,2),min_df=2,max_df=.995,sublinear_tf=True,max_features=35000,strip_accents='unicode')
    Xt=vec.fit_transform(texts[:ntr]); Xv=vec.transform(texts[ntr:]); Xtr=hstack([Xt,csr_matrix(scal[:ntr])],format='csr'); Xte=hstack([Xv,csr_matrix(scal[ntr:])],format='csr')
    ...
    # Strict foldwise prior for meta calibration, so no row's label affects its own prior.
    op=np.zeros(ntr); oc=np.zeros(ntr)
    for a,b in gkf.split(np.zeros(ntr),y,groups):
        p,c,_=objective_prior(train.iloc[a],train.iloc[b],2.0); op[b]=p; oc[b]=c
    tp,tc,gm=objective_prior(train,test,2.0)
    # Meta families: text-only sigmoid, fixed blends, and support-aware clean OOF stack.
    rows=[]; rows.append({'name':'prior_alpha2',**metric(yt,tp)}); rows.append({'name':'retrieved_text_raw',**metric(yt,ptext)})
    for w in [.25,.5,.65,.75,.85]: rows.append({'name':f'blend_text_{w:.2f}',**metric(yt,w*ptext+(1-w)*tp)})
    mcal=LogisticRegression(C=1.0,solver='liblinear').fit(logit(oof).reshape(-1,1),y); pcal=mcal.predict_proba(logit(ptext).reshape(-1,1))[:,1]; rows.append({'name':'text_sigmoid_oof',**metric(yt,pcal)})
    # Support-aware stack lets objective evidence fade automatically when objective support is sparse.
    ...
    meta=LogisticRegression(C=.25,solver='liblinear',max_iter=200).fit(A,y); pm=meta.predict_proba(B)[:,1]; rows.append({'name':'support_aware_oof_stack',**metric(yt,pm)})
    best=min(rows,key=lambda r:r['logloss']); ref=.5778168346923687
    result={'protocol':'V179_KEY_MOMENT_SNIFFS','train_rows':ntr,'test_rows':len(test),'v178_best_reference':ref,'retrieved_vocab':len(vec.vocabulary_),'candidates':rows,'best':best,'delta_vs_v178':best['logloss']-ref,'meta_coef':meta.coef_.ravel().tolist(),'meta_intercept':meta.intercept_.tolist()}
    if best['logloss']<ref-.004: result.update(decision='KEY_MOMENT_PHASE_CHANGE__SCALE_NOW')
    elif best['logloss']<ref: result.update(decision='KEY_MOMENTS_HELP__REFINE_ONE_MORE_SHORT_RUN')
    else: result.update(decision='NO_KEY_MOMENT_GAIN__DO_NOT_SCALE')
    (OUT/'v179_results.json').write_text(json.dumps(result,indent=2)); pd.DataFrame({'response_id':test.response_id.astype(str),'y':yt,'p_text':ptext,'p_prior':tp,'p_meta':pm,'objective_support':tc}).to_csv(OUT/'v179_predictions.csv',index=False); print(json.dumps(result,indent=2))
